...
def mnist4hn(n_patterns):
    ...
    mnist.data /= 255
    perm = np.random.permutation([i for i in range(0,len(mnist.data))])
    train = [mnist.data.to_numpy()[perm[i]] for i in range(n_patterns)] 
    target = [mnist.target.to_numpy()[perm[i]] for i in range(n_patterns)] 
    #train = [mnist.data[0], mnist.data[1], mnist.data[2]]
